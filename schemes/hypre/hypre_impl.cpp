...
Key *HyPRE_Impl::keyGen(Key *public_key, Key *master_key, string identity) {
  ...
  element_init_G2(w_r, pairing);
  element_pow_zn(g_alpha, g2, alpha);
  element_pow_zn(w_r, w, r);
  element_mul(K0, g_alpha, w_r);
  element_pow_zn(K2, g, r);

  res->insertComponent("K0", "G2", K0);
  ...
  element_init_G2(u_ID, pairing);
  element_init_G2(u_ID_h, pairing);
  element_pow_zn(u_ID, u, ID);
  element_mul(u_ID_h, u_ID, h);
  element_pow_zn(K1, u_ID_h, neg_r);
  res->insertComponent("K1", "G2", K1);
...
Key *HyPRE_Impl::keyGen(Key *public_key, Key *master_key, vector<string> *attributes) {
  ...
  for (auto &attribute : *attributes) {
    ...
    element_init_G2(Ktau3, pairing);
    element_t Atau;
    element_init_Zr(Atau, pairing);
    element_t u_Atau, u_Atau_h, u_Atau_h_rtau;
    element_t v_neg_r;
    element_init_G2(u_Atau, pairing);
    element_init_G2(u_Atau_h, pairing);
    element_init_G2(u_Atau_h_rtau, pairing);
    element_init_G2(v_neg_r, pairing);
    ...
    res->insertComponent("K" + attribute + "3", "G2", Ktau3);
  }
...
Ciphertext *HyPRE_Impl::encrypt(element_s *m, string identity, Key *public_key) {
  ...
  element_init_G2(u_ID, pairing);
  element_init_G2(u_ID_h, pairing);
  element_init_G2(u_ID_h_t, pairing);
  element_pow_zn(u_ID, u, ID);
  element_mul(u_ID_h, u_ID, h);
  ...
  element_neg(neg_s, s);
  element_t w_neg_s;
  element_init_G2(w_neg_s, pairing);
  ...
  res->insertComponent("C2", "G2", C2);
  res->insertComponent("C3", "G2", C3);
